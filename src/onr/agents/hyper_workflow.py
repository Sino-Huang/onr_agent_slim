...
def record_planning_intent(
    ...
    joint24_lines = ""
    if context.joint24_trigger_identities is not None:
        host_workspace = context.artifact_root / "workspace" / "001"
        host_workspace.mkdir(parents=True, exist_ok=True)
        materialize_joint24_pddl(
            context.environment_event.payload,
            None,
            context.mission4_gate_decision,
            host_workspace,
            trigger_identities=context.joint24_trigger_identities,
        )
        joint24_lines = (
            "Joint24 code-owned PDDL schedule pre-materialized from current public "
            ...
            f"Schedule metadata for execute: {shell_workspace}/joint24-schedule.json\n"
            ...
            "onr.application.joint24_planning --emit-statechart --plan "
            f"{shell_workspace}/sas_plan --schedule "
            f"{shell_workspace}/joint24-schedule.json --output <the exact "
