...
from onr.runtime.config import StorageConfig, load_runtime_config
from onr.runtime.llm_debug import LLMResponseRecorder


def test_recorder_captures_raw_content_and_reasoning(tmp_path: Path) -> None:
    request_body = {
...
def test_recorder_writes_null_reasoning_and_ignores_other_responses(
    tmp_path: Path,
) -> None:
...
def test_model_without_active_debug_capture_does_not_build_recorder(
    ...
    config = replace(
        load_runtime_config(repo_root=Path(__file__).parents[1]),
        debug=debug,
...
def test_debug_model_owns_mission_scoped_recorder(
    ...
    assert constructed == [tmp_path / "var/debug/llm", "mission:demo"]
