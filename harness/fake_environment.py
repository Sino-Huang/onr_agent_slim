"""A small deterministic environment engine built on the public transport seam.

This module deliberately lives outside ``src/onr``.  It emits evidence for the
environment; it does not apply maneuver feedback to mission state.
"""
...
class FakeEnvironment:
    ...
    def _scene_graph_and_fact(self, command: ManeuverCommand) -> tuple[TransportEvent, TransportEvent]:
        graph = {
        ...
        graph_json = json.dumps(graph, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
        ...
        source_fact_id = f"source-fact:{command.mission_id}:operational_scene_graph:{reference}"
        ...
        if scene_event is None:
            scene_event = self.transport.publish_event(
                ...
                TransportEvent(
                    ...
                    mission_id=command.mission_id,
                    sequence=self.transport.next_event_sequence(self.scene_graph_topic, command.mission_id),
        ...
        if source_fact is None:
            source_fact = self.transport.publish_event(
                ...
                create_source_fact_event(
                    ...
                    sequence=self.transport.next_event_sequence(self.context_topic, command.mission_id),
                    reference=scene_event_id,
                ),
            )
    ...
    def _feedback(self, command: ManeuverCommand, lifecycle: str) -> TransportEvent:
        ...
        event = TransportEvent(
            ...
            sequence=self.transport.next_event_sequence(self.feedback_topic, command.mission_id),
